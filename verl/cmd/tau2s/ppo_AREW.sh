...
USE_AREW_BONUS="${USE_AREW_BONUS:-true}"
TAU2_AREW_LABEL_MODE="${TAU2_AREW_LABEL_MODE:-targeted}"
TAU2_AREW_MIN_TURN="${TAU2_AREW_MIN_TURN:-6}"
AREW_BONUS_MODE="${AREW_BONUS_MODE:-step_abs_sum}"
AREW_BONUS_SCALE="${AREW_BONUS_SCALE:-50.0}"
AREW_NEGATIVE_STEP_MODE="${AREW_NEGATIVE_STEP_MODE:-full}"
AREW_START_STEP="${AREW_START_STEP:-0}"
AREW_END_STEP="${AREW_END_STEP:--1}"

project_name="${DATA_NAME}-${DOMAIN_NAME}-Train"
experiment_name="${EXPERIMENT_NAME:-standard-baseline-14B-ppo-AREW-targeted-v427}"
...
python3 -m verl.trainer.main_ppo \
    ...
    +tau2_arew_label_mode="${TAU2_AREW_LABEL_MODE}" \
    +tau2_arew_min_turn="${TAU2_AREW_MIN_TURN}" \
