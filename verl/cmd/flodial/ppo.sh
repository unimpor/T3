...
CRITIC_MODEL="${CRITIC_MODEL:-${BASE_MODEL}}"
NPC_MODEL="${NPC_MODEL:-${PROJECT_ROOT}/models/Qwen/Qwen2.5-14B-Instruct}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/checkpoints}"
NUM_GPUS="${NUM_GPUS:-8}"
USE_DYNAMIC_BSZ="${USE_DYNAMIC_BSZ:-true}"
TRAIN_FILE="${TRAIN_FILE:-${DATA_DIR}/train_beta_label_opt5_max2.parquet}"
VAL_FILE="${VAL_FILE:-${DATA_DIR}/val_beta_label_opt5_max2.parquet}"

AS_BONUS="${AS_BONUS:-true}"
BT_BONUS="${BT_BONUS:-true}"
BONUS_MODE="${BONUS_MODE:-minority_fixed}"
BONUS_SCALE="${BONUS_SCALE:-0.5}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-Qwen7B-as-bt-max2-scale-0.5-new}"
...
python3 -m verl.trainer.main_ppo \
    ...
    algorithm.btv="${BTV:-v1}" \
    npc.model.path="${NPC_MODEL}" \
    ...
    npc.rollout.name="${NPC_ROLLOUT_NAME:-hf}" \
    npc.rollout.gpu_memory_utilization="${NPC_GPU_MEMORY_UTILIZATION:-0.6}" \
    npc.rollout.n=1 \
    npc.rollout.temperature="${NPC_TEMPERATURE:-1.0}" \
